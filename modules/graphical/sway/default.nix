...
}:
{
  ...
  config = lib.mkIf config.aspects.graphical.sway.enable {
    ...
    programs.regreet = {
      ...
      iconTheme = {
        name = if config.aspects.theme == "dark" then "Papirus-Dark" else "Papirus";
          ...
          in
          {
            ...
            config = rec {
              ...
              startup = [
                ...
                {
                  command = "${pkgs.swaybg}/bin/swaybg -i ${
              ...
              keycodebindings = {
                ...
                "${mod}+Shift+32" =
                  "move container to workspace number 9; workspace $ws9; exec ${pkgs.wtype}/bin/wtype -p ISO_Level5_Latch";
              };
              keybindings = {
                ...
                "${alt}+Print" =
                  ''exec ${pkgs.sway}/bin/swaymsg -t get_tree | ${pkgs.jq}/bin/jq -r '.. | (.nodes? // empty)[] | select(.focused) | "\(.rect.x),\(.rect.y) \(.rect.width)x\(.rect.height)"' | ${pkgs.grim}/bin/grim -g - - | wl-copy'';
